...
import com.cn.mqi.words.processor.*;
import com.cn.mqi.words.util.AnalysisUtil;
...
public class KWSeeker {
	...
	public boolean remove(String word) {
		...
		sensitiveWords.remove(new KeyWord(word));
		reloadKWSeeker();
	...
	/**
	 * 针对批量数据做批量更新, 便于高效率更新树结构
	 */
	/*public boolean addOrDelWord(DataTable dtAddWord, DataTable dtDelWord, String strFieldName) {
		if ((dtAddWord.getCount() == 0 && dtDelWord.getCount() == 0) || MyString.isEmpty(strFieldName)) {
			return false;
		}

		//增量关键词词
		for (int i = 0; i < dtAddWord.getCount(); i++) {
			String strWordValue=Functions.dtCols_strValue(dtAddWord, i, strFieldName);
			if (MyString.isEmpty(strWordValue)==false) {
			    sensitiveWords.add(new KeyWord(strWordValue));
			}
		}
		
		//移除不想要的词
		for (int i = 0; i < dtDelWord.getCount(); i++) {
			String strWordValue=Functions.dtCols_strValue(dtDelWord, i, strFieldName);
			if (MyString.isEmpty(strWordValue)==false) {
				sensitiveWords.remove(new KeyWord(strWordValue));
			}
		}
	...
	public String htmlHighlightWords(String text) {
		return (String) process(new Highlight(), text, new HTMLFragment("<font color='red'>", "</font>"));
	}
	...
	public String toString() {
		return "KWSeeker [sensitiveWords=" + sensitiveWords + ", wordsTree=" + wordsTree + ", wordLeastLen="
				+ wordLeastLen + "]";
	}
